#!/usr/bin/env bash
#============================================================================
# fm.sh - Frontend 개발용 관리 스크립트 (KimsWeb)
#
# 사용법:  ./fm.sh [명령어]
# 예시:    ./fm.sh dev
#============================================================================
...
print_banner() {
    local mode="${SPRING_PROFILES_ACTIVE:-local}"
    echo -e ""
...
do_test() {
    ...
    cd "$FRONTEND_DIR" || exit
    npm run test || warn "test 스크립트가 정의되어 있지 않습니다."
}

do_build() {
    header "Build - 프로덕션 빌드"
    cd "$FRONTEND_DIR" || exit
    local mode="${SPRING_PROFILES_ACTIVE:-production}"
    local vite_mode="$mode"
    if [[ "$vite_mode" == "local" ]]; then
...
do_lint() {
    ...
    cd "$FRONTEND_DIR" || exit
    npm run lint || warn "lint 스크립트가 정의되어 있지 않거나 에러가 있습니다."
}
